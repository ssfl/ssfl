...
def is_counter(vs_main, vs_dependent):
    if (
        not vs_dependent.tt - vs_main.tt
    ):  # The true positive of two set must be disjoint
        return False
    if (
        len(vs_dependent.tf - vs_main.tt) == 0
    ):  # The false positive of dependent must be a subset of main's true positive
...
def construct_join_program(lhs, rhs):
    return JoinProgram([lhs, rhs])
...
def join_version_space_counter(vs_main, vs_dependent):
    ...
    return VersionSpace(
        ...
        tf_,
        ft_,
        construct_counter_program(vs_main.programs[0], vs_dependent.programs[0]),
        out_mappings,
    )


def join_version_space_intersect(vs_main, vs_dependent):
    # Towards the goal of improving precision and recall
    tt_ = vs_dependent.tt.intersection(vs_main.tt)
    tf_ = vs_dependent.tf.intersection(vs_main.tf)
    ft_ = vs_dependent.ft.union(vs_main.ft)
    wr = vs_dependent.programs[0].return_variables[0]
    out_mappings = set()
    for i, (w_bind, r_bind) in vs_dependent.mappings:
        if w_bind[wr] not in vs_main.tt:
            continue
        out_mappings.add((i, (w_bind, r_bind)))
    ...
    return VersionSpace(
        ...
        tf_,
        ft_,
        construct_join_program(vs_main.programs[0], vs_dependent.programs[0]),
        out_mappings,
    )
...
class NoDuplicateLabelConstraintFilter(FilterStrategy):
    def __init__(self, constraint):
        self.constraint_set = set(
            NoDuplicateLabelConstraintFilter.gather_all_constraint(constraint)
        )
        self.word_label = set()
    ...
    def gather_all_constraint(constraint):
        if isinstance(constraint, AndConstraint):
            lhs_constraints = NoDuplicateLabelConstraintFilter.gather_all_constraint(
                constraint.lhs
            )
            rhs_constraints = NoDuplicateLabelConstraintFilter.gather_all_constraint(
                constraint.rhs
            )
            return lhs_constraints + rhs_constraints
        else:
...
def get_valid_cand_find_program(version_space: VersionSpace, program: FindProgram):
    ...
    filterer = CompositeFilter(
        [
            WordAndRelInBoundFilter(program),
            NoDuplicateConstraintFilter(program.constraint),
            NoDuplicateLabelConstraintFilter(program.constraint),
        ]
    )
...
def get_valid_rel_constraint_program(version_space: VersionSpace, program: FindProgram):
    ...
    filterer = CompositeFilter(
        [
            NoDuplicateRelationConstraintFilter(program.relation_constraint),
            WordInBoundFilter(program),
            NoSelfRelationFilter(),
            FixedRelationVarFilter([new_r_var]),
        ]
    )
    candidates = fill_hole(hole, 4, filterer)
...
def get_intersect_rel_vs(
    rc: RelationConstraint, vs, data_sample_set_relation_cache, cache
) -> Set:
    vs_matches = set()
    for i, (w_bind, r_bind) in vs.mappings:
        nx_g = data_sample_set_relation_cache[i]
...
def join_counter_vss(pps, pcps, covered_tt_perfect, new_vss, covered_tt_counter):
    ...
    for vs in new_vss:
        ...
        if target_vs_tf - target_covered_tt:
            continue
        if not (target_vs_tt - target_covered_tt):
            continue
        new_vs_tt = target_vs_tt - target_covered_tt
        new_vs_tt = set([tuple(x) for x in vs.tt if x[-1] not in new_vs_tt])
...
def join_intersect_vss(new_vss, covered_tt_perfect):
    # Goal is to reduce the false positive
    extra_pps = []
    extra_covered_tt = set()
    for i in range(len(new_vss)):
        for j in range(i + 1, len(new_vss)):
            if not new_vss[i].tt.intersection(new_vss[j].tt).issubset(
                covered_tt_perfect | extra_covered_tt
            ) and not new_vss[i].tf.intersection(new_vss[j].tf):
                new_vss[i] = join_version_space_intersect(new_vss[i], new_vss[j])
                extra_pps.append(
                    construct_join_program(
                        new_vss[i].programs[0], new_vss[j].programs[0]
                    )
                )
                extra_covered_tt |= new_vss[i].tt
    return extra_pps, extra_covered_tt
